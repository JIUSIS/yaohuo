...
- 优化首页顶部区域，去掉悬浮菜单，将“我的”和“消息”放到顶部入口。
- 新增双击首页顶部回到顶部。
- 重做“我的”页面为原生界面，修复头像、等级、勋章和资料数据显示异常。
- 修复首页、板块和搜索列表只能加载少量帖子的情况，支持继续加载后续内容。
- 修复帖子从消息、私信、网页、搜索、板块、评论和正文链接进入时可能跳到网页版的问题，现在统一回到原生帖子详情。
- 新增评论区正序、倒序查看。
- 修复帖子新回复通知、消息入口和帖子回复入口跳转不准确的问题。
- 修复悬赏、礼金、获赏和结束状态显示不准确的问题。
- 修复评论区无法显示“谁回复了谁”的问题。
- 修复部分字体会跟随 DPI 异常变大或变小的问题。
- 修复帖子列表外面看不到发帖人的问题。
- 新增原生发帖界面，发帖不再直接使用网页版样式。
- 新增删除帖子功能，并且只在自己的帖子里显示删除按钮。
- 修复回复成功提示，正确显示获得经验和妖晶。
- 修复回复成功但实际没发出去的问题。
- 修复删除回复失效的问题。
- 修复帖子内作者昵称、作者 ID 显示不完整的问题。
- 修复评论区只显示 ID、不显示昵称的问题。
- 去掉回复成功页里的“跳转中...返回”等多余文字。
- 修复帖子链接不高亮、不能点击的问题。
- 修复部分帖子图片加载失败的问题。
- 修复帖子附件显示问题，支持普通附件、图片附件、付费附件。
- 新增评论区头像显示。
- 新增帖子礼金、悬赏、获赏、悬赏状态显示。
